...
/**
 * Portlet implementation class PaymentTransactions
 */
public class PaymentTransactions extends MVCPortlet {
	...
	public void findTransactions(ActionRequest request,ActionResponse response)
		...
		PaymentTransactionsCustomModel paymentTransactionsCustomModel = new PaymentTransactionsCustomModel(transId, accountNumber, serviceTypes, transAmount, paymentMethod, paymentChannel, pmtBankId,new Date(), new Date(), paymentStatus, "",beneficiaryId, beneficiaryServiceNo, serviceId, dateFrom, dateTo);
		...
		List<PaymentTransactionsCustomModel> paymentTransactionsCustomModels = PaymentTransactionsLocalServiceUtil.findCustomPaymentTransactions(paymentTransactionsCustomModel);
		System.out.println("SIZIIZIEIEEISII: " + paymentTransactionsCustomModels.size());
		if (paymentTransactionsCustomModels.size()>0) {
			...
			for (int i=0; i<paymentTransactionsCustomModels.size(); i++) {
				JSONArray record=JSONArray.fromObject(paymentTransactionsCustomModels.get(i), jsonConfig);
