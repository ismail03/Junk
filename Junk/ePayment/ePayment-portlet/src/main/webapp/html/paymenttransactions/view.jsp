<%@page import="com.stc.echannels.epayments.service.PaymentTransactionsLocalServiceUtil"%>
<%@page import="com.stc.echannels.epayments.custommodel.PaymentTransactionsCustomModel"%>
...
<%
	...
	List<String> statuses = PaymentTransactionsLocalServiceUtil.getPaymentStatuses();
 			...
 			<aui:column columnWidth="25" >
		        <aui:select name="paymentMethod">
		        	...
		        	<% for (PaymentMethod pm: paymentMethods) { %>
		        		<aui:option label="<%= (locale==Locale.ENGLISH)?pm.getDescriptiveNameEn():pm.getDescriptiveNameEn()%>" value="<%=pm.getDescriptiveNameEn() %>" /> 
					<% } %>
		        </aui:select>	
 			</aui:column>
  		...
  		<aui:layout>
 			...
 			<aui:column columnWidth="25" >
		        <aui:select name="paymentStatus">
		       		<aui:option label="all" value="" /> 
		        	<% for (String ts: statuses) { %>
		        		<aui:option label="<%=ts %>" value="<%=ts %>" /> 
					 <% } %>
		        </aui:select>	
 			</aui:column>
		...
		if (data !== undefined) {
			jQuery('#dt_results').dataTable( {
				...
				"columns": [
							...
							{"title":"<%=LanguageUtil.get(portletConfig, locale, "transaction-date") %>", "data":"transDate","sClass":"alignCenter"},
							...
							{"title":"<%=LanguageUtil.get(portletConfig, locale, "reason-code") %>", "data":"reason","sClass":"alignCenter"},
							...
							{"title":"<%=LanguageUtil.get(portletConfig, locale, "service-id") %>", "data":"businessService","sClass":"alignCenter"}
 						],
				"pageLength":25,
				"dom": "C<'clear'>Rlfrtip"
			});
